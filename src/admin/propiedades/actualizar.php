...
$queryPropiedades = mysqli_query($db, $consultaPropiedades);
...
$wc = $propiedad['wc'];
$plazas = $propiedad['estacionamiento'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  /* Validamos si existe una imagen en esta petición de actualización */
  ...
  if ($imagenfile['size'] > 1 * 1024 * 1000) {
  ...
  if (empty($errores)) {
    $newImage = ($imagenfile['name']);
    if ($newImage) {
      // Solo intenta eliminar la imagen antigua si se ha subido una nueva
      ...
      // Si no se ha subido una nueva imagen, mantén la antigua
      $nombreArchivo = $imagenActual;
    }
    ...
    if ($newImage) {
      $query .= ", imagen='{$nombreArchivo}'";
    }

    $query .= ", descripcion='{$descripcion}', habitaciones={$habitaciones}, wc={$wc}, estacionamiento={$plazas}, vendedores_id={$vendedor} WHERE id = {$id}";
  ...
  <form action="" method="post" class="space-y-6" enctype="multipart/form-data">
    <fieldset class="border border-gray-300 p-4 rounded-md">
      ...
      <div class="mt-4">
        ...
        <img src="../../imagenes/<?php echo $imagenActual ?>" class="w-48 mt-8">
      </div>
      <div class="mt-4">
    ...
    <button type="submit" class="w-full mt-8 px-6 py-3 bg-blue-500 text-white font-semibold rounded-md hover:bg-blue-600">Actualizar Propiedad</button>
  </form>
</main>
